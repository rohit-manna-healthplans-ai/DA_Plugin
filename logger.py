...
# Track current active window to prevent duplicate click logs on window switch
...
# Event-based screenshot throttling (prevents screenshot floods on rapid clicks/scroll/typing)
EVENT_SS_CLICK_GAP = 1.5     # seconds
EVENT_SS_SCROLL_GAP = 2.5    # seconds
EVENT_SS_TYPED_GAP = 3.0     # seconds
EVENT_SS_PASTE_GAP = 2.0     # seconds

_last_event_ss_by_label = {}

def _event_ss_allowed(label: str, gap: float) -> bool:
    now = time.time()
    last = _last_event_ss_by_label.get(label, 0.0)
    if (now - last) < float(gap):
        return False
    _last_event_ss_by_label[label] = now
    return True
...
class TypedBuffer:
    ...
    def emit(self, chunk, reason, *, backspace: bool, paste: bool):
        ...
        # Event-based screenshot (typed flush)
        if _event_ss_allowed("TYPED_FLUSH", EVENT_SS_TYPED_GAP):
            capture_screenshot(app, title, "TYPED_FLUSH")
...
def on_key_press(k):
    ...
    try:
        ...
        if _ctrl_down and hasattr(k, "char") and (k.char or "").lower() == "v":
            ...
            # Event-based screenshot (paste)
            if _event_ss_allowed("PASTE", EVENT_SS_PASTE_GAP):
                capture_screenshot(app, title, "PASTE")
            return
...
def on_click(x, y, btn, pressed):
    ...
    if pressed:
        ...
        # If window just changed, skip click log (window switch will be logged separately)
        if title == CURRENT_WINDOW_TITLE:
            log_event("MouseClick", f"{btn} at ({x}, {y})", app, title, "Mouse Click")

        # Event-based screenshot (click)
        if _event_ss_allowed("CLICK", EVENT_SS_CLICK_GAP):
            capture_screenshot(app, title, "CLICK")


def on_scroll(x, y, dx, dy):
    # Event-based screenshot (scroll)
    # NOTE: We don't log scroll events to logs to avoid altering existing log semantics.
    app, title = active_window()
    if _event_ss_allowed("SCROLL", EVENT_SS_SCROLL_GAP):
        capture_screenshot(app, title, "SCROLL")
